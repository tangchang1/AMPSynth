...
class ELK_Regressor(nn.Module):
    ...
    def forward(self, esm_embeddings: torch.Tensor, padding_mask: torch.Tensor, bac_ids: torch.Tensor) -> torch.Tensor:
        ...
        pooled_features = self.dropout(pooled_features)  # <--- 加上这一行！
...
def calculate_regression_metrics(y_true, y_pred):
    ...
    # 4. PCC (皮尔逊相关系数)
    # pearsonr 返回 (statistic, pvalue)，我们只需要统计量
    if len(y_true) > 1:
...
def train_model(model, train_loader, valid_loader, criterion, optimizer, device, num_epochs=100, fold_idx=1):
    ...
    best_model_path = f"best_EC_mic_model_fold_{fold_idx}.pth"
...
# 顶刊级可视化画图工具
# ==========================================
def plot_learning_curves(history, fold_idx, save_dir="C:/Users/Mordred/Desktop/single_MIC_Output_Results/EC/plot"):
...
def plot_regression_scatter(y_true, y_pred, fold_idx, pcc_score, save_dir="C:/Users/Mordred/Desktop/single_MIC_Output_Results/EC/Plot"):
    """绘制真实值与预测值的回归散点图"""
    os.makedirs(save_dir, exist_ok=True)
    ...
    # 绘制 y=x 理想红线 (如果点都在这条线上，说明预测100%准确)
...
def run_train():
    print("\n" + "=" * 50)
    print(f"  [TRAIN MODE] 启动： ECMIC 回归预测")
    print("=" * 50)
    # --- 第一步：读取数据 ---
    ...
    gc.collect()
    torch.cuda.empty_cache()
    print(f" 特征融合完毕！当前输入矩阵维度: {global_embs.shape} (1280 + 4)")
    ...
    for fold_idx, (train_idx_in_cv, valid_idx_in_cv) in enumerate(gkf.split(cv_idx, y_cv, groups_cv)):
        ...
        # 设定保存路径 (结合你原有的 OUTPUT_DIR 和折数)
        csv_filename = os.path.join(OUTPUT_DIR, f"EC_Fold_{fold_idx + 1}_Test_Predictions.csv")
        df_scatter.to_csv(csv_filename, index=False)
        print(f" Fold {fold_idx + 1} 的散点图数据已成功保存至: {csv_filename}")
        ...
        #  呼叫全新的散点图绘制函数
        plot_regression_scatter(all_t, all_p, fold_idx + 1, t_pcc)
...
def run_predict(target_bacteria_id: int = 0):
    ...
    if target_bacteria_id not in BACTERIA_NAMES:
        print(f"❌ 错误：细菌编号必须是 0, 1, 2, 3 中的一个！您当前输入的是 {target_bacteria_id}")
        return
    ...
    predict_loader = DataLoader(predict_dataset, batch_size=512, shuffle=False, collate_fn=feature_collate_fn,
                                pin_memory=True)
    # --- 加载回归模型 ---
    model_path = R"E:\Users\Mordred\Desktop\single_MIC_Output_Results\EC\best_EC_mic_model.pth"
...
# ---  文件路径配置 ---
OUTPUT_DIR = "E:/Users/Mordred/Desktop/single_MIC_Output_Results/EC"
os.makedirs(OUTPUT_DIR, exist_ok=True)
MASTER_MIC_EXCEL_PATH = "E:/Users/Mordred/Desktop/EC.xlsx"  # 样本数据
NEW_PEPTIDES_PATH = "E:/Users/Mordred/Desktop/C.xlsx"  # 未来给 predict 模式留的待测数据
