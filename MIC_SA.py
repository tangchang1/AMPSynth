...
def train_model(model, train_loader, valid_loader, criterion, optimizer, device, num_epochs=100, fold_idx=1):
    ...
    best_model_path = f"best_SA_mic_model_fold_{fold_idx}.pth"
...
# 顶刊级可视化画图工具
# ==========================================
def plot_learning_curves(history, fold_idx, save_dir="C:/Users/Mordred/Desktop/single_MIC_Output_Results/SA/plot"):
...
def plot_regression_scatter(y_true, y_pred, fold_idx, pcc_score, save_dir="C:/Users/Mordred/Desktop/single_MIC_Output_Results/SA/Plot"):
    """绘制真实值与预测值的回归散点图"""
    os.makedirs(save_dir, exist_ok=True)
...
def run_train():
    print("\n" + "=" * 50)
    print(f"  [TRAIN MODE] 启动： SA_MIC 回归预测")
    print("=" * 50)
    # --- 第一步：读取数据 ---
    ...
    for fold_idx, (train_idx_in_cv, valid_idx_in_cv) in enumerate(gkf.split(cv_idx, y_cv, groups_cv)):
        ...
        csv_filename = os.path.join(OUTPUT_DIR, f"SA_Fold_{fold_idx + 1}_Test_Predictions.csv")
...
def run_predict(target_bacteria_id: int = 0):
    ...
    predict_loader = DataLoader(predict_dataset, batch_size=512, shuffle=False, collate_fn=feature_collate_fn,
                                pin_memory=True)
    # --- 加载回归模型 ---
    model_path = R"E:\Users\Mordred\Desktop\single_MIC_Output_Results\SA\best_SA_mic_model.pth"
...
# ---  文件路径配置 ---
OUTPUT_DIR = "E:/Users/Mordred/Desktop/single_MIC_Output_Results/SA"
os.makedirs(OUTPUT_DIR, exist_ok=True)
MASTER_MIC_EXCEL_PATH = "E:/Users/Mordred/Desktop/SA.xlsx"  # 样本数据
